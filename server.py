...
def remix():
    ...
    try:
        ...
        remix_type = data.get('remix-type', 'blog')  # Default to 'blog' if not provided
        print(f"Received: {prompt[:50]}..., Type: {remix_type}")

        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={API_KEY}"
        payload = {
            "contents": [{
                "parts": [{"text": f"{prompt} - Please generate a very long, detailed response in the style of a {remix_type} with extensive content, examples, and depth."}]
            }],
            "generation_config": {
                "max_output_tokens": 2048,  # Ensure long output
                "temperature": 0.7
            }
        }
